use poprako_entity::comic::{ActiveModel, Column, Entity, Model};
use sea_orm::{ActiveModelTrait, ConnectionTrait, DbErr, EntityTrait, PaginatorTrait, QueryOrder};

/// # 功能
/// comic_table 仓储入口，聚合 `comic_table` 表的 CRUD 与按 ID 查询。
...
impl ComicRepository {
    /// # 功能
    /// 读取 `comic_table` 全部行，按 `created_at` 升序返回。
    ///
    /// ## 参数
