use poprako_entity::role_request::{ActiveModel, Column, Entity, Model};
use sea_orm::{ActiveModelTrait, ConnectionTrait, DbErr, EntityTrait, PaginatorTrait, QueryOrder};

/// # 功能
/// role_request_table 仓储入口，聚合 `role_request_table` 表的 CRUD 与按 ID 查询。
/// 写操作单方法内独立提交，不自动开启事务；批量场景由 Service 包事务。
///
/// ## 关联
/// - 表：`role_request_table`
pub struct RoleRequestRepository;

impl RoleRequestRepository {
    /// # 功能
    /// 读取 `role_request_table` 全部行，按 `created_at` 升序返回。
    ///
    /// ## 参数
